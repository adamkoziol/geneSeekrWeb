...
sub rawMapping {
	...
	make_path("$path/SipprResults/tmp");
	chdir ("$path/uploads");
	my $name = "$file" . "_$geneName";
	# Perform smalt mapping
	unless (-e ("$path/SipprResults/tmp/$name.sam")) {
		system("smalt map -f samsoft -o $path/SipprResults/tmp/$name.sam -n 24 $path/SipprTarget/$geneName $rawFile");
	}
